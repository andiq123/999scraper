...
export const vehicleCatalog: readonly VehicleBrand[] = [
  ...
  {
    canonical: 'GAC',
    aliases: ['gac', 'gac motor'],
    models: ['Aion S', 'Aion V', 'Aion Y', 'GS3', 'GS8', 'Hyptec HT'],
  },
  {
    canonical: 'Geely',
    aliases: ['geely'],
    models: ['Atlas', 'Atlas Pro', 'Coolray', 'Emgrand', 'Galaxy E5', 'Galaxy L7', 'Geometry C', 'Monjaro', 'Tugella'],
  },
  ...
  { canonical: 'Jaecoo', aliases: ['jaecoo'], models: ['J7', 'J8'] },
  {
    canonical: 'Jetour',
    aliases: ['jetour'],
    models: ['Dashing', 'G700', 'T1', 'T1 i-DM', 'T2', 'T2 i-DM', 'X50', 'X70', 'X70 Plus', 'X90', 'X90 Plus', 'X95'],
  },
  ...
  {
    canonical: 'Lynk & Co',
    aliases: ['lynk and co', 'lynk & co', 'lynk co'],
    models: ['01', '02', '03', '05', '06', '08', '09', 'Z10'],
  },
  { canonical: 'Luxeed', aliases: ['luxeed'], models: ['R7', 'S7'] },
  { canonical: 'Maextro', aliases: ['maextro'], models: ['S800'] },
  {
    canonical: 'Maxus',
    aliases: ['maxus', 'saic maxus'],
    models: ['D60', 'D90', 'eDeliver 3', 'Euniq 5', 'Euniq 6', 'Mifa 9', 'T90'],
  },
...
const makePhrases = vehicleCatalog
  .flatMap((brand) => brand.aliases)
  .filter((alias) => alias.includes(' ') || alias.includes('&'));

export const vehicleEntityCompletions: ReadonlyArray<{ canonical: string; aliases: readonly string[] }> =
...
export function matchesVehicleCatalog(plain: string, words: readonly string[]): boolean {
  return (
    words.some((word) => singleWordMakes.has(word) || classicStandaloneModels.has(word)) ||
