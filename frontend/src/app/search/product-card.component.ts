...
export class ProductCardComponent {
  ...
  readonly changeState = input<'new' | 'removed' | 'price' | null>(null);
  readonly previousPrice = input('');
  ...
  readonly metadata = computed(() => {
    ...
    const vehicle = [
      product.mileage && `${product.mileage.toLocaleString('ro-MD')} km`,
      product.originCountry,
      product.fuel,
      product.transmission,
      product.drivetrain,
      product.bodyType,
      product.power && `${product.power} hp`,
    ];
    const property = [
      product.sector,
      product.offerType,
      product.rooms,
      product.area && (/^\d+(?:[.,]\d+)?$/.test(product.area) ? `${product.area} m²` : product.area),
      product.floor && `Floor ${product.floor}`,
    ];
    const technology = [
